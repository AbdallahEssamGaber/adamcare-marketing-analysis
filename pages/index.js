...
import { MOCK_POSTS, getMockPostsByMonth, getAvailableMockMonths } from '@/lib/mockData';
...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && localStorage.getItem('foradam_auth') === 'true') {
      setAuthenticated(true);
    }
  }, []);

  // Available months
  const availableMonths = useMemo(() => getAvailableMockMonths(), []);
  ...
  const posts = useMemo(() => {
    ...
    return getMockPostsByMonth(selectedMonth.year, selectedMonth.month);
  }, [selectedMonth]);

  // Previous month posts for delta
  const prevPosts = useMemo(() => {
    if (!selectedMonth) return [];
    let pm = selectedMonth.month - 1;
    let py = selectedMonth.year;
    if (pm === 0) { pm = 12; py--; }
    return getMockPostsByMonth(py, pm);
  }, [selectedMonth]);

  // Metrics
  const metrics = useMemo(() => {
    if (posts.length === 0) return { totalViews: 0, avgReach: 0, avgEngagementRate: 0, totalPosts: 0 };
    ...
    return { totalViews, avgReach, avgEngagementRate: Number(avgEngagementRate), totalPosts: posts.length };
  }, [posts]);

  const prevMetrics = useMemo(() => {
    if (prevPosts.length === 0) return null;
    const totalViews = prevPosts.reduce((s, p) => s + p.views, 0);
    const avgReach = Math.round(prevPosts.reduce((s, p) => s + p.reach, 0) / prevPosts.length);
    const totalEngagement = prevPosts.reduce((s, p) => s + p.likes + p.comments + p.shares + p.saves, 0);
    const avgEngagementRate = totalViews > 0 ? ((totalEngagement / totalViews) * 100).toFixed(2) : 0;
    return { totalViews, avgReach, avgEngagementRate: Number(avgEngagementRate), totalPosts: prevPosts.length };
  }, [prevPosts]);
  ...
  const weeklyData = useMemo(() => {
    ...
    posts.forEach((p) => {
      ...
      if (!weeks[weekIdx].tiktok) weeks[weekIdx] = { week: `Week ${weekIdx + 1}`, tiktok: 0, instagram: 0 };
  ...
  const platformSplit = useMemo(() => {
    if (posts.length === 0) return [];
    const ttViews = posts.filter((p) => p.platform === 'tiktok').reduce((s, p) => s + p.views, 0);
    const igViews = posts.filter((p) => p.platform === 'instagram').reduce((s, p) => s + p.views, 0);
    return [
  ...
  return (
    <>
      ...
      {syncMessage && (
        <div style={{ textAlign: 'center', padding: '8px', fontSize: '13px', color: '#555' }}>
          {syncMessage}
        </div>
      )}
      ...
      <div className="dashboard">
        ...
        <div className="metrics">
          {[
            { label: 'Total Views', value: metrics.totalViews, prev: prevMetrics?.totalViews, format: true },
            { label: 'Avg Reach / Post', value: metrics.avgReach, prev: prevMetrics?.avgReach, format: true },
            { label: 'Avg Engagement Rate', value: metrics.avgEngagementRate, prev: prevMetrics?.avgEngagementRate, suffix: '%' },
            { label: 'Total Posts', value: metrics.totalPosts, prev: prevMetrics?.totalPosts },
          ].map((card) => {
            ...
            return (
              <div className="metric-card" key={card.label}>
                ...
                <div className="metric-value">
                  {card.format ? formatNumber(card.value) : card.value}{card.suffix || ''}
                </div>
                {delta && (
                  <div className={`metric-delta ${delta.positive ? 'positive' : 'negative'}`}>
                    {delta.positive ? '+' : ''}{delta.value}% vs last month
                  </div>
                )}
              </div>
            );
          ...
          <div className="chart-card full">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={weeklyData}>
                ...
                <Legend />
                <Line type="monotone" dataKey="tiktok" name="TikTok" stroke="#333333" strokeWidth={2} dot={{ r: 4 }} />
                <Line type="monotone" dataKey="instagram" name="Instagram" stroke="#C2185B" strokeWidth={2} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="chart-card">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={engagementData}>
                ...
                <Tooltip formatter={(v) => formatNumber(v)} />
                <Legend />
                <Bar dataKey="tiktok" name="TikTok" fill="#333333" radius={[4, 4, 0, 0]} />
                <Bar dataKey="instagram" name="Instagram" fill="#C2185B" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="top-posts">
          ...
          {topPosts.map((post) => (
            <div className="post-row" key={post.post_id}>
              ...
              <a className="post-link" href={post.link} target="_blank" rel="noopener noreferrer">
                View
              </a>
            </div>
          ))}
        ...
        <div className="insights-section">
          ...
          <button className="btn btn-insights" onClick={handleInsights} disabled={insightsLoading}>
